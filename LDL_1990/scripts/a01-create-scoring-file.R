...
strand_flip <- function(allele){
  ...
  gds <- seqOpen( paste0("/home/mok36/imputation_code/freeze9b/replication/14_replication_make_gds/replication-9b-samoa-chr", chr.i, ".dose.gds"))  # freeze 9 
  
  load("/home/jnc35/gwas/phenotypes/replication/phenotypes_replication_jenna_2023-05-31.RData")
  
  # keep replication 1990 subjects only 
  sample.id <- seqGetData(gds, "sample.id")
  keep.id <- sample.id[which(sample.id %in% phenotypes.gwas.annot.1990$sample.id)]
  ...
  save(final.scoring.file, file=paste0("~/PRS/LDL_1990/scoring-files/final.scoring.file.chr", chr.i, ".RData"))
  save(final.dos, file=paste0("~/PRS/LDL_1990/dosages/final.dos.chr", chr.i))
