...
strand_flip <- function(allele){
  ...
  # keep replication 2002 subjects only 
  sample.id <- seqGetData(gds, "sample.id")
  keep.id <- sample.id[which(sample.id %in% phenotypes.gwas.annot.2002$sample.id)]
  ...
  save(final.scoring.file, file=paste0("~/PRS/LDL_2002/scoring-files/final.scoring.file.chr", chr.i, ".RData"))
  save(final.dos, file=paste0("~/PRS/LDL_2002/dosages/final.dos.chr", chr.i))
